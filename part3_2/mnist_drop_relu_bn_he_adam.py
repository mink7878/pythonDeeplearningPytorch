# -*- coding: utf-8 -*-
"""mnist_drop_relu_bn_he_adam.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1W4B0p0ZioQH-dmeUUflGXvFNjCD2xiK1
"""
...
# He initializtion 이용해 파라미터 초기화하기
import torch.nn.init as init

def weight_init(m): # MLP의 weight를 초기화할 부분을 설정하기 위한 함수 정의
  if isinstance(m, nn.Linear): # MLP를 구성하고 있는 파라미터 중 nn.Linear에 해당하는 파라미터 값에 대해서만 지정
    init.kaiming_uniform_(m.weight.data) # nn.Linear에 해당하는 파라미터 값에 대해 he init 이용해 초기화

model = Net().to(DEVICE)
model.apply(weight_init)
optimizer = torch.optim.Adam(model.parameters()) # Adam = RMSProp + momentum
