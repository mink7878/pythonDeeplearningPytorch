...
Original file is located at
    https://colab.research.google.com/drive/18dHkXQSrv_5JrSthA-VNzq8a3wR2jp17
"""
...
"""- 이론상 dropout을 적용했을 때 일반화가 강해져 test accuracy가 높아지는 결과가 기대되지만, 이는 __학습 데이터셋과 검증 데이터셋의 피처 및 레이블의 분포 간 많은 차이__가 있을 때 유효하게 작용한다.
- MNIST 데이터셋은 학습 데이터와 검증 데이터 간 많은 차이가 발생 X  
  => 오히려 성능이 조금 하락할 수 있음!! (but, __epoch 늘려 추가로 학습 진행__하면 성능 좋아지는 경향 있음.)
"""