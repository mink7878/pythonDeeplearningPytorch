...
Original file is located at
    https://colab.research.google.com/drive/1x5LKB8JJ37GZsKQKWtdmThxNB5z-CrZf

- 흑백 이미지 데이터 MNIST 같은 경우, 기본적인 MLP 모델을 이용할 경우 90% 정확도 모델 구현 가능
- 하지만 컬러 이미지인 cifar-10의 경우 MLP 모델 이용할 경우 49%의 정확도  
> 이유  
> : MINIST, cifar-10 모두 MLP 모델의 input으로 이용될 때 이미지를 1차원으로 펼쳐 이용된다. 즉, 이미지 내에 지역적인 특징을 반영할 수 없는 한계점이 있다.  
> : 흑백 데이터인 MNIST는 구조가 단순해 비교적 덜 민감 & 채널 수도 1이기 때문에 1차원으로 펼쳐 이용되는 것에 덜 민감.  
> : cifar-10은 컬러 이미지이기 때문에 채널 수 3이며, 32 * 32 크기의 2차원 이미지가 3개 겹쳐 있어 모양을 형성하는 이미지를 32 * 32 * 3의 1차원으로 펼쳐 놓으므로 이미지의 특징을 잃어버리는 것에 더욱 민감.  

- 따라서 1차원으로 펼쳐 input으로 이용하는 것이 아니라, 이미지와 주변 픽셀의 조합을 통해 특징을 추출하고자 하는 CNN 모형을 제안하게 됐음.
"""
...
"""- 앞서 설계한 MLP 모델은 49% 수준에 그친 반면, conv 연산을 추가한 CNN 모델은 성능이 약 63% 수준으로 향상된 것을 확인할 수 있다."""